...
Esta coleção percorre o ciclo de vida completo da API Maratona contra um ambiente **real**: cadastro de usuários e times, consultas, edições e limpeza. Tudo bate em PostgreSQL, MongoDB, Neo4j, Redis e na API do Codeforces de verdade.
...
* **Usuário (só o dono da conta):** altera nome de exibição, e-mail, `nomeUsuario` (e desfaz) e senha. Trocar o `nomeUsuario` invalida o token antigo, porque o *subject* do JWT muda. A resposta já devolve um token novo, e a coleção o captura.
...
* **`colecao.json`** é a coleção `TestesAPIMaratona`.
* **`ambiente.json`** guarda as variáveis `url`, `token_jwt` e `idProblema`. Só `url` precisa de atenção (padrão `http://localhost:8080`); as outras duas são preenchidas em tempo de execução.
